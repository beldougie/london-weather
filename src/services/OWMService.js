...
export default class OWMService {
   ...
   * 
   * @param {String} [units] opttional switch for celsius/fahrenheit temperature scales. Defaults to celsius
   * @returns {Promise}
   */
  getCurrent(units = 'metric') {
    if(units !== 'metric' && units !== 'imperial') units = 'metric'
    return fetch(this.buildUrl('weather', `q=${this.city}&units=${units}`))
